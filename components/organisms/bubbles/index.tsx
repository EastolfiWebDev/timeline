...
export default function Bubbles(props) {
            ...
            setBubbles([
                {
                    ...
                    isImage: false,
                    comment:
                        '&nbsp;When you tried a new tool!&nbsp;<br>by nickolasrm',
                },
            ]);
    }, [bubbles, setBubbles]);
    ...
    function addBubble(identifier) {
        const newBubbles = [];
        bubbles.forEach(el => {
            if (el.identifier !== identifier) newBubbles.push(el);
            else {
                newBubbles.push(el);
                newBubbles.push({
                    identifier: nextId(),
    ...
    function updateBubble(identifier, content) {
        setBubbles(
            bubbles.map(el => {
                if (el.identifier === identifier) el = { ...el, ...content };
                return el;
            }),
        );
    }
    ...
    return (
        <div
            className={'disable-trans ' + styles.ContainerStyle}
