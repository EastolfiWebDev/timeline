...
export default function Comment(props) {
    ...
    useEffect(() => {
        const handleChange = () => {
            if (isImage) setImage(comment);
            else if (
                textareaRef.current &&
                comment !== textareaRef.current.innerHTML
            )
    ...
    useEffect(() => {
        if (textareaRef && comment) textareaRef.current.innerHTML = comment;
    ...
    return (
        <div
            className={
                ...
                ' disable-anim ' +
                (props.isEven
                    ? styles.EvenCommentStyle
                    : styles.OddCommentStyle)
            }
            data-testid="comment">
            {!image ? (
                <div
            ...
            <CommentImageButton
                ...
                image={image}
                className={styles.HoveringButtonsStyle}></CommentImageButton>
        </div>
    );
}
